...
{
    // A compact scientific calculator inspired by Casio fx-580 layout and common functions.
    public class CalculatorForm : Form
    {
        // native caret not used (we render our own caret indicator)
        private Panel screen;
        private int caretIndex = 0;
        private Panel caretIndicator;
        private Timer caretTimer;
        private bool caretVisibleState = true;
        private string lastAnswer = "0";
        private string expressionBacking = ""; // single-line parseable expression
        private int visibleStartIndex = 0; // start index of the substring shown on screen
        private int visibleTopOffset = 0; // pixel offset to vertically center lines inside screen

        private int GetLineHeight()
        {
            try { return (screen?.Font.Height ?? 16) + 2; }
            catch { return 18; }
        }
        private bool caretNavScrolling = false; // when true, UpdateVisibleText scrolls minimally to keep caret visible
        ...
        {
            base.OnShown(e);
            // place caret at end when the form appears
            // Defer visual updates until after initial layout to get correct measurements
        ...
        private void InitializeComponent()
        {
            ...
            // Single screen (Label) - we render text and manage caret ourselves
            screen = new Panel();
            screen.Width = this.ClientSize.Width - 24;
            screen.Location = new Point(12, 12);
            // make screen taller to accomodate up to 4 lines
            screen.Height = 40 * 4 + 8;
            screen.Font = new Font("Consolas", 18, FontStyle.Bold);
            screen.Padding = new Padding(6, 6, 6, 6);
            screen.BackColor = Color.FromArgb(230, 240, 245);
            screen.ForeColor = Color.FromArgb(20, 20, 20);
            screen.BorderStyle = BorderStyle.FixedSingle;
            expressionBacking = "0";
            screen.TabStop = true;
            screen.Paint += (s, ev) =>
            {
                ev.Graphics.Clear(screen.BackColor);
                ev.Graphics.TextRenderingHint = System.Drawing.Text.TextRenderingHint.ClearTypeGridFit;
                int controlInnerWidth = screen.ClientSize.Width - screen.Padding.Left - screen.Padding.Right - 6;
                int lineHeight = GetLineHeight();
                // Build visible lines the same way UpdateVisibleText does (we rely on visibleStartIndex and visibleTopOffset)
                var lines = (screen.Tag as List<string>) ?? new List<string> { "" };
                for (int i = 0; i < lines.Count; i++)
                {
                    string line = lines[i];
                    var size = ev.Graphics.MeasureString(line, screen.Font);
                    int x = screen.Padding.Left + Math.Max(0, controlInnerWidth - (int)size.Width) + 6;
                    int y = screen.Padding.Top + visibleTopOffset + i * lineHeight;
                    using (var brush = new SolidBrush(screen.ForeColor))
                    {
                        ev.Graphics.DrawString(line, screen.Font, brush, x, y);
                    }
                }
            };
            this.Controls.Add(screen);

            // Caret visual indicator (blinking) so users see edit position clearly
            caretIndicator = new Panel();
            caretIndicator.Width = 2;
