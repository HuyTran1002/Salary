    ...
    public class CalculatorForm : Form
    {
        ...
        private void CalculatorForm_KeyDown(object? sender, KeyEventArgs e)
        {
            ...
            // Escape = xóa hết
                ...
                key = "AC";
            // ( và )
            else if (e.KeyCode == Keys.D9 && e.Shift)  // Shift + 9 = (
                key = "(";
            else if (e.KeyCode == Keys.D0 && e.Shift)  // Shift + 0 = )
                key = ")";
            // ^ (mũ)
            else if (e.KeyCode == Keys.D6 && e.Shift)  // Shift + 6 = ^
                key = "^";
            // % (phần trăm)
            else if (e.KeyCode == Keys.D5 && e.Shift)  // Shift + 5 = %
                key = "%";
            // Ans (phím A)
            else if (e.KeyCode == Keys.A && e.Control)  // Ctrl + A
                key = "Ans";
            // Up/Down điều hướng history
            ...
            {
                // Search for button in all panels
                ...
                foreach (Control ctrl in this.Controls)
                {
                    if (ctrl is TableLayoutPanel panel)
                    {
                        ...
                        {
                            if (panelCtrl is Button btn && (btn.Tag as string) == key)
                            {
                                foundBtn = btn;
                                break;
                            }
