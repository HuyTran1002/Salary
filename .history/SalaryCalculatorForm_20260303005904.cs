...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
            ...
            {
                "Lương này nhìn khá cân đối giữa công sức và thu nhập",
                "Mức này ổn, giữ đều vài tháng là rất đẹp",
                "Lương đang có xu hướng tăng, tín hiệu tích cực",
                "Thu nhập này đủ tốt để lập quỹ dự phòng",
                "Mức net này khá ổn để giữ nhịp tài chính",
                "Lương tháng này có chất lượng đó nha",
                "Con số này cho thấy bạn làm việc hiệu quả",
                "Mức này nếu tối ưu thuế sẽ còn đẹp hơn",
                "Lương hiện tại khá cạnh tranh rồi",
                "Thu nhập này đáng để tự thưởng một bữa ngon",
                "Mức này ổn áp, chỉ cần giữ sức bền",
                "Lương tháng này nhìn có động lực ghê",
                "Con số này nói lên sự cố gắng rõ ràng",
                "Mức net này đang đi đúng mục tiêu",
                "Thu nhập này phù hợp để lên kế hoạch dài hạn",
                "Mức này tốt, cân nhắc tăng kỹ năng để bật tiếp",
                "Lương ổn, nhưng vẫn còn dư địa cải thiện",
                "Con số này rất đáng ghi nhận",
                "Mức này mà quản lý chi tiêu tốt là quá đẹp",
                "Lương tháng này đủ sức kéo mood lên cao",
                "Thu nhập này phản ánh năng suất tốt",
                "Mức này đáng để tiếp tục duy trì phong độ",
                "Lương khá sáng, đặc biệt nếu OT hợp lý",
                "Con số này ổn định và đáng tin cậy",
                "Mức lương này cho thấy bạn đang tiến bộ",
                "Thu nhập này có thể đặt mục tiêu cao hơn",
                "Mức này khá đẹp so với mặt bằng chung",
                "Lương này nếu giữ đều sẽ rất an tâm",
                "Con số này đủ để xây nền tài chính tốt",
                "Mức net hiện tại khá hợp lý và thực tế",
                "Lương này nhìn là thấy tháng làm việc chất lượng",
                "Thu nhập này ổn để cân bằng sống và làm việc",
                "Mức này đáng khen, tiếp tục phát huy",
                "Lương tháng này có đà, đừng bỏ nhịp",
                "Con số này đẹp theo kiểu bền vững",
                "Mức này nếu thêm phụ cấp sẽ còn ngon",
                "Thu nhập khá ổn, giữ nhịp học thêm kỹ năng",
                "Lương hiện tại có thể gọi là rất ổn",
                "Con số này đang ủng hộ kế hoạch tiết kiệm",
                "Mức này đủ tự tin để đặt KPI mới",
                "Lương tháng này nhìn chắc chắn và thực dụng",
                "Thu nhập như này là thành quả rõ rệt",
                "Mức này rất hợp để tối ưu tài chính cá nhân",
                "Lương ổn, nhớ giữ sức khỏe để đi đường dài",
                "Con số này là bước đệm tốt cho tăng trưởng",
                "Mức net này đáng để mỉm cười",
                "Thu nhập này khá ổn để mở rộng mục tiêu",
                "Mức này cho thấy bạn đang đi đúng hướng",
                "Lương tháng này có chiều sâu, không chỉ bề nổi",
                "Con số này đẹp và có tiềm năng tăng tiếp"
            };
        ...
        private void EnsureStickPhysicsRecovery(Rectangle bounds)
        {
            ...
            if (stickFigurePosition.Y >= bounds.Bottom - 0.5f)
            {
                ...
                {
                    stickAction = Math.Abs(stickVelocityX) > 2.6f ? "Chạy" : "Đi bộ";
                }
            }
        }
        ...
        private Rectangle GetStickWalkBounds()
        {
            ...
            int groundBottom = stickFigureHostPanel.ClientSize.Height - 10;
            try
            {
                var resultPanelFound = stickFigureHostPanel.Controls.Find("resultPanel", true);
                if (resultPanelFound.Length > 0)
                {
                    Rectangle resultRect = GetBoundsRelativeTo(resultPanelFound[0], stickFigureHostPanel);
                    if (!resultRect.IsEmpty)
                    {
                        groundBottom = Math.Min(groundBottom, resultRect.Top - 14);
                    }
                }
            }
            catch
            ...
            int bottom = Math.Max(top + 100, groundBottom);
        ...
        private string BuildSmartSpeech(Control target)
        {
            ...
            if (target == null)
            {
                ...
                if (netValue >= 25000000m) return GetRandomSalaryComment();
            ...
            {
                if (netValue >= 25000000m) return GetRandomSalaryComment();
                if (netValue >= 15000000m) return "Net ổn áp, có thể đẩy thêm chút OT";
