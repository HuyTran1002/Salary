...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
        ...
        private string GenerateSmartContextMessage(Control target, decimal netValue, decimal taxPercent, decimal overtime3x, decimal overtime2x, int emptyKeyFields)
        {
            ...
            if (stickUserPattern.ContainsKey("frequent_ot3x") && stickUserPattern["frequent_ot3x"] >= 3)
            {
                if (stickRandom.NextDouble() < 0.4)
                    return $"Mình nhận ra bạn hay nhập OT x3 nhiều lắm! Kiên trì quá đó 💪{currentMood}";
            }

            if (stickUserPattern.ContainsKey("high_tax") && stickUserPattern["high_tax"] >= 2)
            {
                if (stickRandom.NextDouble() < 0.3)
                    return $"Mình thấy bạn luôn chú ý tối ưu thuế. Bạn rất tỉnh thức! 🎯{currentMood}";
            }
            ...
            if (target is TextBox tb)
            {
                ...
                if (name.Contains("overtime3x") && overtime3x > 0)
                {
                    return new[] {
                        ...
                        "OT x3 - mình là AI mà cũng thấy mệt! 😅",
                        "Số tiếng OT x3 này... wow, bạn chi quá! 🔥"
                    }[stickRandom.Next(4)];
                }
            ...
            {
                return $"Net tầm này... bạn đang ở mốc cao rồi! Mình dự đoán bạn sẽ đạt nhiều mục tiêu 🎊{currentMood}";
            }

            if (emptyKeyFields >= 5)
            {
                return "Mình thấy còn khá nhiều ô để điền. Từ từ, mình sẽ giáo dục từng buước đó! 📚";
            }
            ...
            // Fall back to original system but filtered
            string randomMsg = GetRandomDialogue();
            while (stickSpeechHistory.Contains(randomMsg) && stickSpeechHistory.Count >= 3)
            {
                randomMsg = GetRandomDialogue();
            }
            return randomMsg;
        }
