            ...
            try
            {
                ...
                parsingTasks.Add(System.Threading.Tasks.Task.Run(async () => {
                    try {
                        ...
                        bool shouldSyncFromSheet = false;
                        
                        if (now.Day < 21) {
                            // Trước ngày 21: Bảng chứa dữ liệu tháng hiện tại (21 tháng trước - 20 tháng này)
                            // Nên sync nếu tính lương tháng hiện tại
                            if (m == now.Month && y == now.Year) {
                                shouldSyncFromSheet = true;
                            }
                        } else {
                            // Từ ngày 21 trở đi: Bảng chứa dữ liệu tháng tiếp theo (21 tháng này - 20 tháng sau)
                            // Nên sync nếu tính lương tháng tiếp theo
                            int nextMonth = now.Month == 12 ? 1 : now.Month + 1;
                            int nextYear = now.Month == 12 ? now.Year + 1 : now.Year;
                            if (m == nextMonth && y == nextYear) {
                                shouldSyncFromSheet = true;
                            }
                        }
                        
                        if (shouldSyncFromSheet) {
                            // Sync từ sheet và lưu cache
                            ...
                            // Tải từ cache
                            LoadLeaveDataCache(username, m, y, out decimal cachedAl, out decimal cachedSl);
                            sAl = cachedAl;
                            sSl = cachedSl;
                        }
                    } catch { }
