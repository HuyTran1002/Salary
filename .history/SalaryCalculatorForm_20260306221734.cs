    ...
    public partial class SalaryCalculatorForm : Form
    {
                          ...
                          TextBox allowanceTextBox, TextBox recognizeTextBox, TextBox otherBonusTextBox, TextBox taxThresholdTextBox)
        {
            try
            {
                ...
                string detail = $"CHI TIẾT TÍNH LƯƠNG THÁNG {month:D2}/{year}\n" +
                    $"========================================\n\n" +

                    $"THÔNG TIN CƠ BẢN:\n" +
                    $"- Lương cơ bản: {basicSalary:C0} VND\n" +
                    $"- Số ngày công: {workingDays:F1} ngày\n" +
                    $"- Ngày nghỉ SL: {slDaysOff:F1} ngày\n" +
                    $"- Ngày nghỉ AL: {alDaysOff:F1} ngày\n\n" +
                    ...
                    $"- Lương ngày công sau trừ nghỉ: {regularSalary:C0} VND\n\n" +

                    $"LÀM THÊM GIỜ (OT):\n" +
                    $"- OT x2 ({overtime2xHours:F1} giờ × {hourlyRate:C0} × 2): {overtime2xSalary:C0} VND\n" +
                    $"- OT x3 ({overtime3xHours:F1} giờ × {hourlyRate:C0} × 3): {overtime3xSalary:C0} VND\n" +
                    $"- OT x1.5 ({overtime15xHours:F1} giờ × {hourlyRate:C0} × 1.5): {overtime15xSalary:C0} VND\n" +
                    $"- Tổng OT: {overtime2xSalary + overtime3xSalary + overtime15xSalary:C0} VND{bonusInfo}\n\n" +

                    $"PHỤ CẤP & THƯỞNG:\n" +
                    $"- Tiền đi lại ({allowanceEligibleDays:F1} ngày × {travelPerDay:C0}): {travelAllowance:C0} VND\n" +
                    $"- Tiền chuyên cần ({allowanceEligibleDays:F1} ngày × {attendancePerDay:C0}): {attendanceIncentive:C0} VND\n" +
                    $"- Tiền nhà ở: {housingAllowance:C0} VND\n" +
                    (ratingBonus > 0 ? $"- Thưởng xếp loại ({selectedRating}): {ratingBonus:C0} VND\n" : "") +
                    (certificateBonus > 0 ? $"- Thưởng chứng chỉ: {certificateBonus:C0} VND\n" : "") +
                    (recognizeCount > 0 ? $"- Recognize ({recognizeCount} × 50,000): {recognizeCount * 50000:C0} VND\n" : "") +
                    (otherBonus > 0 ? $"- Thưởng khác: {otherBonus:C0} VND\n" : "") +
                    $"- Tổng phụ cấp & thưởng: {allowanceTotal:C0} VND\n\n" +

                    $"TỔNG HỢP:\n" +
                    $"- Lương Brutto: {grossSalary:C0} VND\n" +
                    $"- Khấu trừ BH ({insurancePercent:F1}% × {basicSalary:C0}): -{insuranceDeduction:C0} VND\n" +
                    $"- Thu nhập trước thuế: {netSalaryBeforeTax:C0} VND\n" +
                    $"- Khấu trừ thuế: -{taxDeduction:C0} VND\n" +
                    $"- Lương Net (thực nhận): {netSalary:C0} VND\n\n" +

                    $"THÔNG TIN THÊM:\n" +
                    $"- Mốc thuế áp dụng: {taxThreshold:C0} VND\n" +
                    $"- Thuế suất: {taxRate:P1}\n" +
                    $"- Tỷ giá giờ: {hourlyRate:C0}/giờ\n" +
                    $"- Tiền ăn/ngày: {mealDailySalary:C0} VND";

                // Save calculation to user data
                int month = int.Parse(monthTextBox.Text);
                int year = int.Parse(yearTextBox.Text);
                userDataManager.UpdateLastCalculation(currentUsername, month, year, netSalary, detail);
