...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
        ...
        private void StickFigureHostPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            PointF shoulder = new PointF(stickFigurePosition.X + lean, bodyTopY + 6f);
            PointF hip = new PointF(stickFigurePosition.X + lean * 0.45f, bodyBottomY);

            PointF leftHandTarget = new PointF(stickFigurePosition.X - 11f, bodyTopY + 8f + stride * 2.5f);
            PointF rightHandTarget = new PointF(stickFigurePosition.X + 11f, bodyTopY + 8f - stride * 2.5f);
            PointF leftFootTarget = new PointF(stickFigurePosition.X - 8f * direction, stickFigurePosition.Y + 11f + stride * 1.2f);
            PointF rightFootTarget = new PointF(stickFigurePosition.X + 8f * direction, stickFigurePosition.Y + 11f - stride * 1.2f);
            ...
            if (stickAction == "Chạy")
            {
                ...
                leftHandTarget = new PointF(stickFigurePosition.X - 14f, bodyTopY + 7f - sprint * 0.5f);
                rightHandTarget = new PointF(stickFigurePosition.X + 14f, bodyTopY + 7f + sprint * 0.5f);
                leftFootTarget = new PointF(stickFigurePosition.X - 10f * direction, stickFigurePosition.Y + 11f + sprint);
                rightFootTarget = new PointF(stickFigurePosition.X + 10f * direction, stickFigurePosition.Y + 11f - sprint);
            }
            else if (stickAction == "Nhảy")
            {
                float kick = (float)Math.Sin(stickWalkPhase * 1.2f) * 2f;
                leftHandTarget = new PointF(stickFigurePosition.X - 9f, bodyTopY - 12f);
                rightHandTarget = new PointF(stickFigurePosition.X + 9f, bodyTopY - 12f);
                leftFootTarget = new PointF(stickFigurePosition.X - 5f, stickFigurePosition.Y + 7f - kick);
                rightFootTarget = new PointF(stickFigurePosition.X + 5f, stickFigurePosition.Y + 7f + kick);
            }
            else if (stickAction == "Leo trèo")
            {
                float climbWave = (float)Math.Sin(stickWalkPhase * 1.4f) * 5f;
                leftHandTarget = new PointF(stickFigurePosition.X - 6f, bodyTopY + 0.5f + climbWave);
                rightHandTarget = new PointF(stickFigurePosition.X + 6f, bodyTopY + 9f - climbWave);
                leftFootTarget = new PointF(stickFigurePosition.X - 5f, stickFigurePosition.Y + 8f - climbWave * 0.55f);
                rightFootTarget = new PointF(stickFigurePosition.X + 5f, stickFigurePosition.Y + 8f + climbWave * 0.55f);
            }
            else if (stickAction == "Tương tác")
            {
                float wave = (float)Math.Sin(stickGesturePhase * 1.8f) * 3f;
                leftHandTarget = new PointF(stickFigurePosition.X - 10f, bodyTopY + 6.5f + wave);
            }
            ...
            using (var jointBrush = new SolidBrush(Color.FromArgb(18, 18, 18)))
            {
                ...
                g.DrawLine(bodyPen, shoulder.X - 4.2f, shoulder.Y - 0.8f, shoulder.X + 4.2f, shoulder.Y - 0.8f);
                g.DrawLine(bodyPen, hip.X - 3.6f, hip.Y, hip.X + 3.6f, hip.Y);

                DrawJointedLimb(g, limbPen, jointBrush, shoulder, leftElbow, leftHandTarget, 1.9f);
                DrawJointedLimb(g, limbPen, jointBrush, shoulder, rightElbow, rightHandTarget, 1.9f);
                DrawJointedLimb(g, limbPen, jointBrush, hip, leftKnee, leftFootTarget, 2.1f);
                DrawJointedLimb(g, limbPen, jointBrush, hip, rightKnee, rightFootTarget, 2.1f);
                ...
                g.FillEllipse(Brushes.White, eyeX - 1.05f, headCenterY - 1.1f, 2.1f, 2.1f);
            }
