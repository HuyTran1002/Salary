...
namespace SalaryCalculator
{
    ...
    public class CalculatorForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            // Row 0: ↑ (col 1 only) - Navigation Up
            AddBtn("↑", 1, 0, tag: "UP", bgColor: navColor);

            // Row 1: ← → (Remote TV style, ↓ moved to row 7)
            AddBtn("←", 0, 1, tag: "LEFT", bgColor: navColor);
            AddBtn("→", 2, 1, tag: "RIGHT", bgColor: navColor);
            AddBtn("Ans", 3, 1, tag: "Ans", bgColor: opColor);  // Moved here to fill gap
            ...
            // Row 6: 0 (không span, bằng phím số khác), ., +, %
            AddBtn("0", 0, 6);
            AddBtn(".", 1, 6, tag: ".");
            AddBtn("+", 2, 6, tag: "+", bgColor: opColor);
            AddBtn("%", 3, 6, tag: "%", bgColor: opColor);

            // Row 7: ↓, ^, = (dài nhất - spanning 2 cols)
            AddBtn("↓", 0, 7, tag: "DOWN", bgColor: navColor);
            AddBtn("^", 1, 7, tag: "^", bgColor: opColor);
            AddBtn("=", 2, 7, colSpan: 2, tag: "=", bgColor: eqColor);
