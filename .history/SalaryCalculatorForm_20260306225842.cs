...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            // Create overlay panel for stick figure (on top of everything)
            var stickOverlayPanel = new Panel
            {
            ...
            stickFigureOverlayPanel = stickOverlayPanel;
            
            // Set double buffer on overlay
            stickOverlayPanel.DoubleBuffered = true;
            
            this.Controls.Add(stickOverlayPanel);
            stickOverlayPanel.BringToFront();
            ...
            try { InitializeStickFigureCompanion(stickOverlayPanel, mainPanel); } catch { }
        }

        private void InitializeStickFigureCompanion(Panel overlayPanel, Panel mainPanel)
        {
            if (overlayPanel == null || overlayPanel.IsDisposed || mainPanel == null || mainPanel.IsDisposed)
            {
                return;
            }

            stickFigureHostPanel = overlayPanel;
            stickFigurePosition = new PointF(28f, Math.Max(80f, overlayPanel.Height * 0.35f));
            ...
            EnableDoubleBuffer(overlayPanel);
            overlayPanel.Paint -= StickFigureHostPanel_Paint;
            overlayPanel.Paint += StickFigureHostPanel_Paint;
            WireStickClickHandlers(mainPanel);

            mainPanel.ControlAdded += (s, e) =>
            {
                if (e.Control != null)
                {
    ...
    public class TransparentPanel : Panel
    {
        ...
        protected override void OnPaintBackground(PaintEventArgs e)
        {
            // Don't paint background to remain transparent
        }

        protected override void OnPaint(PaintEventArgs e)
        {
            // Allow the base paint (which calls Paint subscribers like StickFigureHostPanel_Paint)
            base.OnPaint(e);
        }
    }
}
