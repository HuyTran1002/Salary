...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            overtime2xResultLabel.Text = "OT FWD: 0 VND | OT x2: 0 VND";
            overtime2xResultLabel.Location = new System.Drawing.Point(190, rightY);
            overtime2xResultLabel.Width = 410;
        ...
        private void UpdateOvertime2xDisplay()
        {
            try
            {
                ...
                // Calculate salaries
                decimal otFwdSalary = Math.Round(otFwdHours * hourlyRate, 0, MidpointRounding.AwayFromZero);
                decimal ot2xSalary = Math.Round(ot2xHours * hourlyRate * 2, 0, MidpointRounding.AwayFromZero);

                // Update label text
                overtime2xResultLabel.Text = $"OT FWD: {otFwdSalary:C0} | OT x2: {ot2xSalary:C0}";
            }
            catch
            {
                          ...
                          TextBox allowanceTextBox, TextBox recognizeTextBox, TextBox otherBonusTextBox, TextBox taxThresholdTextBox)
        {
            try
            {
                ...
                // Split OT x2 into OT FWD (<=24 hours at 1x rate) and OT x2 (remaining at 2x rate)
                decimal otFwdHours = Math.Min(overtime2xHours, 24);
                decimal ot2xHours = Math.Max(0, overtime2xHours - 24);
                decimal otFwdSalary = Math.Round(otFwdHours * hourlyRate, 0, MidpointRounding.AwayFromZero);
                decimal ot2xSalary = Math.Round(ot2xHours * hourlyRate * 2, 0, MidpointRounding.AwayFromZero);
                ...
                string detail = $"Chi Tiết:\n" +
                    ...
                    $"  • OT FWD ({otFwdHours:F1} tiếng × {hourlyRate:C0}): {otFwdSalary:C0} VND\n" +
