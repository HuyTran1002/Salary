    ...
    public partial class SalaryCalculatorForm : Form
    {
        ...
        private void InitializeStickFigureCompanion(Panel hostPanel)
        {
            ...
            if (stickFigureOverlayPanel != null)
            {
                try
                {
                    stickFigureOverlayPanel.Paint -= StickFigureHostPanel_Paint;
            ...
            stickFigureOverlayPanel = new StickFigureOverlayPanel();
            stickFigureOverlayPanel.Name = "stickFigureOverlayPanel";
            stickFigureOverlayPanel.Dock = DockStyle.Fill;
            stickFigureOverlayPanel.TabStop = false;
            stickFigureOverlayPanel.Paint += StickFigureHostPanel_Paint;
            EnableDoubleBuffer(hostPanel);
            hostPanel.Controls.Add(stickFigureOverlayPanel);
            stickFigureOverlayPanel.BringToFront();

            hostPanel.ControlAdded += (s, e) =>
            {
                if (stickFigureOverlayPanel != null && !stickFigureOverlayPanel.IsDisposed)
                {
                    stickFigureOverlayPanel.BringToFront();
                }
            };
            ...
            hostPanel.Resize += (s, e) =>
            {
                if (!hostPanel.IsDisposed)
                {
                    ...
                    if (stickFigureOverlayPanel != null && !stickFigureOverlayPanel.IsDisposed)
                    {
                        stickFigureOverlayPanel.Invalidate();
                    }
                }
            };
        ...
        private void UpdateStickFigureState()
        {
            ...
            if (stickFigureOverlayPanel != null && !stickFigureOverlayPanel.IsDisposed)
            {
                Rectangle newBounds = ComputeStickDrawBounds();
                Rectangle dirty = Rectangle.Union(stickLastDrawBounds, newBounds);
                ...
                dirty.Inflate(8, 8);
                stickFigureOverlayPanel.Invalidate(dirty);
                stickLastDrawBounds = newBounds;
            }
        }
        ...
        private Rectangle ComputeStickDrawBounds()
        {
            ...
            if (stickFigureOverlayPanel != null && !stickFigureOverlayPanel.IsDisposed)
            {
                Rectangle clientRect = new Rectangle(Point.Empty, stickFigureOverlayPanel.ClientSize);
        ...
        private string BuildSmartSpeech(Control target)
        {
            ...
            if (target == null)
            {
                if (netValue >= 25000000m) return "Net cao đẹp quá, giữ phong độ nha!";
            ...
            if (target is TextBox tb)
            {
                ...
                {
                    return overtime3x > 0 ? "OT x3 ngon nè, nhưng giữ sức nữa nha" : "Có OT x3 thì net sẽ bật mạnh đó";
                }
                if (lowerName.Contains("tax"))
                {
                    ...
                    return "Mức thuế này đang trong vùng kiểm soát";
                }
                if (lowerName.Contains("salary"))
                {
                    return "Lương cơ bản là gốc, mọi thứ tính từ đây";
                }
            ...
            {
                if (netValue >= 25000000m) return "Net này quá đẹp, top performance luôn";
