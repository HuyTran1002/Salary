...
namespace SalaryCalculator
{
    ...
    public class CalculatorForm : Form
    {
        ...
        private double EvaluateExpression(string expr)
        {
            try
            {
                ...
                while (expr.Contains("("))
                {
                    ...
                    expr = expr.Substring(0, lastOpen) + innerResult.ToString("G17", CultureInfo.InvariantCulture) + expr.Substring(firstClose + 1);
                }
        ...
        private double SimpleEvaluate(string expr)
        {
            ...
            // Helper function to find number boundaries
            Func<string, int, (int start, int end)> findNumberBoundaries = (s, opIdx) =>
            {
                // Find left number
                int leftStart = opIdx - 1;
                while (leftStart > 0 && (char.IsDigit(s[leftStart - 1]) || s[leftStart - 1] == '.'))
                    leftStart--;
                
                // Find right number  
                int rightEnd = opIdx + 1;
                while (rightEnd < s.Length - 1 && (char.IsDigit(s[rightEnd + 1]) || s[rightEnd + 1] == '.'))
                    rightEnd++;
            ...
            while (expr.Contains("^"))
            {
                ...
                var (leftStart, rightEnd) = findNumberBoundaries(expr, idx);
            ...
            while (expr.Contains("*") || expr.Contains("/"))
            {
                ...
                expr = expr.Substring(0, leftStart) + result.ToString("G17", CultureInfo.InvariantCulture) + expr.Substring(rightEnd + 1);
            }
