...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
        ...
        private void StickFigureHostPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            PointF leftArmDefault = new PointF(stickFigurePosition.X - 9f, bodyTopY + 6f + walkOffset);
            PointF rightArmDefault = new PointF(stickFigurePosition.X + 9f, bodyTopY + 6f - walkOffset);
            PointF rightArmEnd = rightArmDefault;
            PointF leftLegEnd = new PointF(stickFigurePosition.X - 6f * direction, stickFigurePosition.Y + 10f);
            PointF rightLegEnd = new PointF(stickFigurePosition.X + 6f * direction, stickFigurePosition.Y + 10f + walkOffset * 0.3f);
            ...
            if (hasVisibleTarget)
            {
                ...
                if (dist > 0.001f)
                {
                    ...
                    rightArmEnd = new PointF(
                        rightArmDefault.X + (stretched.X - rightArmDefault.X) * stickHandReach,
                        rightArmDefault.Y + (stretched.Y - rightArmDefault.Y) * stickHandReach);
                }
            }
            ...
            if (stickAction == "Chạy")
            {
                ...
                leftLegEnd = new PointF(stickFigurePosition.X - 7f * direction, stickFigurePosition.Y + 10f + sprint);
                rightLegEnd = new PointF(stickFigurePosition.X + 7f * direction, stickFigurePosition.Y + 10f - sprint);
            }
            else if (stickAction == "Nhảy")
            {
                leftLegEnd = new PointF(stickFigurePosition.X - 4f, stickFigurePosition.Y + 7f);
                rightLegEnd = new PointF(stickFigurePosition.X + 4f, stickFigurePosition.Y + 7f);
            }
            else if (stickAction == "Leo trèo")
            {
                float climbWave = (float)Math.Sin(stickWalkPhase * 1.4f) * 5f;
                leftArmDefault = new PointF(stickFigurePosition.X - 5f, bodyTopY + 2f + climbWave);
                rightArmDefault = new PointF(stickFigurePosition.X + 5f, bodyTopY + 8f - climbWave);
                rightArmEnd = rightArmDefault;
                leftLegEnd = new PointF(stickFigurePosition.X - 5f, stickFigurePosition.Y + 8f - climbWave * 0.5f);
                rightLegEnd = new PointF(stickFigurePosition.X + 5f, stickFigurePosition.Y + 8f + climbWave * 0.5f);
            }
            ...
            using (var headBrush = new SolidBrush(Color.FromArgb(225, 243, 255)))
            {
                ...
                float wave = (float)Math.Sin(stickGesturePhase) * 2.8f;
                g.DrawLine(accentPen, shoulder, new PointF(leftArmDefault.X, leftArmDefault.Y + wave));
                g.DrawLine(accentPen, shoulder, rightArmEnd);
                g.FillEllipse(Brushes.White, rightArmEnd.X - 1.7f, rightArmEnd.Y - 1.7f, 3.4f, 3.4f);
                g.DrawEllipse(bodyPen, rightArmEnd.X - 1.7f, rightArmEnd.Y - 1.7f, 3.4f, 3.4f);

                g.DrawLine(bodyPen, new PointF(stickFigurePosition.X, bodyBottomY), leftLegEnd);
                g.DrawLine(bodyPen, new PointF(stickFigurePosition.X, bodyBottomY), rightLegEnd);
            ...
            using (var moodFont = new Font("Arial", 7f, FontStyle.Bold))
            {
                string badge = $"{stickMood} • {stickAction}";
                var badgeRect = new Rectangle((int)stickFigurePosition.X - 40, (int)stickFigurePosition.Y + 14, 92, 16);
                using (var bg = new SolidBrush(Color.FromArgb(165, 232, 244, 255)))
                using (var border = new Pen(Color.FromArgb(95, 88, 132), 1f))
                {
                    g.FillRectangle(bg, badgeRect);
                    g.DrawRectangle(border, badgeRect);
                }
                TextRenderer.DrawText(g, badge, moodFont, badgeRect, Color.FromArgb(43, 63, 106), TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis);
            }
        }
