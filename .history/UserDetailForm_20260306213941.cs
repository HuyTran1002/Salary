...
namespace SalaryCalculator
{
    public class UserDetailForm : Form
    {
        ...
        public UserDetailForm(UserInfo user)
        {
            ...
            {
                foreach (var entry in user.SalaryHistory.OrderByDescending(e => ParseHistoryKey(e.Key)))
                {
                    bool hasDetail = user.SalaryInputHistory != null && user.SalaryInputHistory.ContainsKey(entry.Key);
                    salaryGrid.Rows.Add(
                        entry.Key,
                        entry.Value.ToString("N0") + " VND",
                        hasDetail ? "Bấm để xem input tháng này" : "Dữ liệu cũ (chưa có input chi tiết)");
                }
            }
            else
            {
            ...
            salaryGrid.CellClick += (s, e) =>
            {
                ...
                if (user.SalaryInputHistory != null && user.SalaryInputHistory.TryGetValue(periodKey, out SalaryCalculationDetail detail))
                {
                    var detailForm = new SalaryHistoryDetailForm(user, periodKey, detail);
                    detailForm.ShowDialog(this);
                }
                else
                {
                    MessageBox.Show(
                        "Dòng lịch sử này là dữ liệu cũ nên chưa có chi tiết input. Hãy tính lương lại tháng đó để lưu đầy đủ input.",
                        "Chưa có chi tiết input",
