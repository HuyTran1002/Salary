...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
        ...
        // Try to play an applause (vỗ tay) sound. Prefer embedded resource named applause.wav or external Assets/audio/applause.wav
        private void PlayApplauseIfAvailable()
        {
            try
            {
                bool played = false;
                ...
                if (!played)
                {
                    ...
                    var candidates = new List<string>()
                    {
                        System.IO.Path.Combine(appDir, "applause.wav"),
                        System.IO.Path.Combine(appDir, "Assets", "audio", "applause.wav"),
                        System.IO.Path.Combine(appDir, "Assets", "audio", "clap.wav"),
                        System.IO.Path.Combine(appDir, "Assets", "audio", "applause.mp3")
                    };
                    try
                    {
                        var found = System.IO.Directory.GetFiles(appDir, "applause.*", System.IO.SearchOption.AllDirectories).FirstOrDefault();
                        if (!string.IsNullOrEmpty(found)) candidates.Insert(0, found);
                    }
                    catch { }
                    ...
                    foreach (var path in candidates)
                    {
                        try
                        {
                            ...
                            // SoundPlayer supports WAV. For MP3, try launching a background player using system call to default app
                            if (System.IO.Path.GetExtension(path).ToLowerInvariant() == ".wav")
                            {
                            ...
                            {
                                // For MP3, try to use Windows Media Player COM via shell execute (fire and forget)
                                try
                                {
