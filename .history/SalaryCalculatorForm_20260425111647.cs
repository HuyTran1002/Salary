            ...
            try
            {
                ...
                parsingTasks.Add(System.Threading.Tasks.Task.Run(async () => {
                    try {
                        ...
                        if (canSyncFromSheet) {
                            // Đồng bộ từ sheet vì bảng chứa tháng m
                            ...
                            // Lưu dữ liệu vào cache để tính lương lần sau
                            SaveLeaveDataCache(username, m, y, sAl, sSl);
                        } else {
                            // Bảng không chứa tháng m, tải từ cache
                            if (!LoadLeaveDataCache(username, m, y, out decimal cachedAl, out decimal cachedSl)) {
                                // Cache trống: có thể tháng chưa bao giờ lưu, giữ giá trị 0
                                sAl = 0;
                                sSl = 0;
                            } else {
                                sAl = cachedAl;
