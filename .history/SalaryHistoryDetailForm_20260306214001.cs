...
namespace SalaryCalculator
{
    public class SalaryHistoryDetailForm : Form
    {
        public SalaryHistoryDetailForm(UserInfo user, string periodKey, SalaryCalculationDetail detail)
        {
            ...
            detailBox.Text =
                "THÔNG TIN CHUNG" + Environment.NewLine +
                $"- Nhân viên: {detail.EmployeeName}" + Environment.NewLine +
                $"- Kỳ lương: {detail.Month:D2}-{detail.Year}" + Environment.NewLine +
                $"- Thời điểm tính: {detail.CalculatedAt:dd/MM/yyyy HH:mm:ss}" + Environment.NewLine + Environment.NewLine +

                "DỮ LIỆU ĐÃ NHẬP TỪ BẢNG TÍNH LƯƠNG" + Environment.NewLine +
                $"- Lương cơ bản: {detail.BasicSalary:N0} VND" + Environment.NewLine +
                $"- Tiền ăn/ngày công: {detail.MealAllowancePerDay:N0} VND" + Environment.NewLine +
                $"- Số ngày công: {detail.WorkingDays:N1}" + Environment.NewLine +
                $"- Nghỉ SL: {detail.SlDaysOff:N1} ngày" + Environment.NewLine +
                $"- Nghỉ AL/PH: {detail.AlDaysOff:N1} ngày" + Environment.NewLine +
                ...
                $"- Ngày OT +4h: {detail.OtDays8:N1} ngày" + Environment.NewLine +
                ...
                $"- % Bảo hiểm: {detail.InsurancePercent:N2}%" + Environment.NewLine +
                $"- % Thuế áp dụng: {detail.TaxPercent:N2}%" + Environment.NewLine +
                $"- Mốc lương tính thuế (input): {detail.TaxThresholdInput:N0} VND" + Environment.NewLine +
                ...
                $"- Bonus khác: {detail.OtherBonus:N0} VND" + Environment.NewLine + Environment.NewLine +

                "THAM SỐ PHỤ CẤP / CẤU HÌNH LIÊN QUAN" + Environment.NewLine +
                ...
                $"- Tiền nhà ở: {detail.HousingAllowance:N0} VND" + Environment.NewLine +
                ...
                "KẾT QUẢ" + Environment.NewLine +
                $"- Lương Brutto: {detail.GrossSalary:N0} VND" + Environment.NewLine +
                $"- Lương Net: {detail.NetSalary:N0} VND";
