...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
            ...
            private Timer netBlinkTimer = null;
            private int netBlinkColorIndex = 0;
            ...
            // AI-Smart Dialog System với khả năng học hỏi lâu dài
            private Queue<string> stickSpeechHistory = new Queue<string>(5);
            private Dictionary<string, int> stickUserPattern = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
            private Dictionary<string, double> responseEffectiveness = new Dictionary<string, double>(StringComparer.OrdinalIgnoreCase);
            private int stickTotalSpeechCount = 0;
            private int totalSessions = 0;
            private DateTime stickLastMajorInteraction = DateTime.Now;
            private string aiLearningFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ai_learning.json");
            private string stickLastSpeechMessage = "";
            private DateTime stickLastSpeechTime = DateTime.Now;
            private DateTime? stickLastInteraction = null;
            ...
            private Color[] marqueeColors = new Color[]
            {
                ...
                Color.FromArgb(236, 250, 236),
                Color.FromArgb(232, 248, 255),
                Color.FromArgb(236, 240, 255),
                Color.FromArgb(246, 236, 255)
            };
            private Color[] netBlinkColors = new Color[]
            {
                Color.Red,
                Color.Orange,
                Color.Goldenrod,
        ...
        private void InitializeComponent()
        {
            if (currentUsername == "dev")
            {
                ...
                devTitle.Text = "⚙️  CẤU HÌNH HỆ THỐNG";
                devTitle.Font = new System.Drawing.Font("Arial", 14, System.Drawing.FontStyle.Bold);
                devTitle.ForeColor = Color.FromArgb(214, 236, 255);
                devTitle.Dock = DockStyle.Top;
                devTitle.TextAlign = System.Drawing.ContentAlignment.MiddleCenter;
                devTitle.Height = 40;
                this.Controls.Add(devTitle);
                ...
                saveSheetBtn.BackColor = Color.FromArgb(37, 110, 255);
                saveSheetBtn.ForeColor = Color.White;
                saveSheetBtn.FlatStyle = FlatStyle.Flat;
                saveSheetBtn.FlatAppearance.BorderSize = 0;
                saveSheetBtn.Cursor = Cursors.Hand;
                saveSheetBtn.Click += (s, e) =>
                {
                    var (docId, gid) = ParseSheetUrl(sheetUrlBox.Text.Trim());
                    if (string.IsNullOrEmpty(docId))
                    {
                        saveStatusLabel.ForeColor = Color.Tomato;
                        saveStatusLabel.Text = "❌ Link không hợp lệ, hãy kiểm tra lại.";
                        return;
                    }
                    string normalized = $"https://docs.google.com/spreadsheets/d/{docId}/edit?gid={gid}";
                    var cfg = AppSettings.Load();
                    cfg.CompanySheetUrl = normalized;
                    cfg.Save();
                    saveStatusLabel.ForeColor = Color.LimeGreen;
                    saveStatusLabel.Text = $"✅ Đã lưu! GID: {gid}";
                };
                devPanel.Controls.Add(saveSheetBtn);

                // Reset to default button
                Button resetSheetBtn = new Button();
                resetSheetBtn.Text = "↩ Mặc định";
                resetSheetBtn.Location = new Point(14, 200);
                resetSheetBtn.Width = 120;
                resetSheetBtn.Height = 26;
                resetSheetBtn.Font = new System.Drawing.Font("Arial", 8);
                resetSheetBtn.BackColor = Color.FromArgb(60, 70, 100);
                resetSheetBtn.ForeColor = Color.FromArgb(180, 200, 240);
                resetSheetBtn.FlatStyle = FlatStyle.Flat;
                resetSheetBtn.FlatAppearance.BorderSize = 0;
                resetSheetBtn.Cursor = Cursors.Hand;
                resetSheetBtn.Click += (s, e) =>
                {
                    sheetUrlBox.Text = "https://docs.google.com/spreadsheets/d/1msmu_9Cy8JxwyDYAgEGAJ82qy5cWGOzXQpDzqyE2lWM/edit?gid=306468592";
                    saveStatusLabel.Text = "";
                };
                devPanel.Controls.Add(resetSheetBtn);

                this.Controls.Add(devPanel);
                return;
            }
            ...
            marqueeLabel.ForeColor = Color.Black;
            marqueeLabel.BackColor = marqueeColors[0];
            ...
            // Initialize marquee with loading text
            marqueeText = "🚀  Đang tải bảng xếp hạng tháng " + DateTime.Now.Month.ToString("D2") + "...";
            
            // Calculate real ranking in background to prevent UI block
            System.Threading.Tasks.Task.Run(() => {
                try {
                    string realText = GetTop5RankingText();
                    this.BeginInvoke((MethodInvoker)delegate {
                        if (marqueeLabel != null && !marqueeLabel.IsDisposed) {
                            marqueeText = realText;
                            marqueeX = marqueeLabel.Width + 24; // Reset position
                        }
                    });
                } catch { }
            });
            ...
            Size marqueeTextSize = TextRenderer.MeasureText(
                marqueeText,
                marqueeLabel.Font,
            ...
            marqueeTimer.Tick += (s, e) =>
            {
                ...
                if (marqueeX < -(marqueeTextSize.Width + marqueeHorizontalPadding))
                {
                    marqueeX = marqueeLabel.Width + marqueeHorizontalPadding; // Reset to right side with padding
                }

                // Blink background through 7 soft tones, keep text black
                tickCount++;
                if (tickCount % 5 == 0)
                {
            ...
            marqueeLabel.Paint += (s, e) =>
            {
                ...
                int textY = (marqueeLabel.Height - marqueeTextSize.Height) / 2;
                TextRenderer.DrawText(
                    e.Graphics,
                    marqueeText,
                    marqueeLabel.Font,
                    new Point(marqueeX, textY),
                    marqueeLabel.ForeColor,
            ...
            this.FormClosing += (s, e) =>
            {
                ...
                if (netBlinkTimer != null)
                {
                    netBlinkTimer.Stop();
                    netBlinkTimer.Dispose();
                    netBlinkTimer = null;
                }
            };
            ...
            empNameLabel.Font = new System.Drawing.Font("Arial", 12, System.Drawing.FontStyle.Bold);
            ...
            float detailFont = 11.5f;
            float detailFontBold = 11.5f;
            ...
            netLabel.ForeColor = netBlinkColors[0];
            netLabel.Location = new System.Drawing.Point(10, detailY);
            netLabel.Width = 400;
            netLabel.Height = 20;
            netLabel.Name = "netLabel";

            netBlinkTimer = new Timer();
            netBlinkTimer.Interval = 180;
            netBlinkTimer.Tick += (s, e) =>
            {
                if (netLabel == null || netLabel.IsDisposed || !netLabel.IsHandleCreated)
                {
                    return;
                }

                netBlinkColorIndex = (netBlinkColorIndex + 1) % netBlinkColors.Length;
                netLabel.ForeColor = netBlinkColors[netBlinkColorIndex];
            };
            netBlinkTimer.Start();
            ...
            detailTitleLabel.Font = new System.Drawing.Font("Arial", 10, System.Drawing.FontStyle.Bold);
            ...
            detailLabel.Font = new System.Drawing.Font("Arial", 9.5f);
            ...
            // Setup AI learning event handlers for TextBox interactions
            SetupAILearningHandlers();
        ...
        private void InitializeStickFigureCompanion(Panel hostPanel)
        {
            ...
            this.FormClosing += (s, e) =>
            {
                try
                {
                    SaveAILearning(); // Save AI learning data before closing
                    if (stickFigureTimer != null)
                    {
                        stickFigureTimer.Stop();
                        stickFigureTimer.Dispose();
        ...
        private void UpdateStickFigureState()
        {
            ...
            if (stickInteractionTicks > 0)
            {
                ...
                {
                    string newSpeech = BuildSmartSpeech(stickActiveTarget);
                    stickCurrentSpeech = newSpeech;
                    stickLastSpeechMessage = newSpeech;
                    stickLastSpeechTime = DateTime.Now;
                    stickSpeechTicks = 58;
        ...
        private void LoadAILearning()
        {
            try
            {
                if (File.Exists(aiLearningFile))
                {
                    string json = File.ReadAllText(aiLearningFile);
                    var learning = System.Text.Json.JsonSerializer.Deserialize<AILearningData>(json);
                    if (learning != null)
                    {
                        stickUserPattern = learning.UserPatterns ?? new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
                        responseEffectiveness = learning.ResponseEffectiveness ?? new Dictionary<string, double>(StringComparer.OrdinalIgnoreCase);
            ...
            {
                // Reset if corrupted
                stickUserPattern = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
                responseEffectiveness = new Dictionary<string, double>(StringComparer.OrdinalIgnoreCase);
                totalSessions = 0;
            }
        }

        private void SaveAILearning()
        {
            try
            {
                var learning = new AILearningData
                {
                    UserPatterns = stickUserPattern,
                    ResponseEffectiveness = responseEffectiveness,
                    TotalSessions = totalSessions + 1,
                    LastUpdated = DateTime.Now
                };
                string json = System.Text.Json.JsonSerializer.Serialize(learning, new System.Text.Json.JsonSerializerOptions { WriteIndented = true });
            ...
            try
            {
                ...
                parsingTasks.Add(System.Threading.Tasks.Task.Run(async () => {
                    try {
                        ...
                        // Nếu tìm thấy dữ liệu từ sheet, lưu vào cache
                        if (dataFoundFromSheet) {
                            SaveOTDataCache(username, m, y, sumOt15, sumOt2, sumFw, sumOt3, d12.Count, d8.Count);
                        } else {
                            // Nếu không có dữ liệu từ sheet, tải từ cache
                            if (LoadOTDataCache(username, m, y, out decimal cachedOt15, out decimal cachedOt2, out decimal cachedFw, out decimal cachedOt3, out int cachedD12, out int cachedD8)) {
                                ...
                                for (int i = 0; i < cachedD12; i++) d12.Add("placeholder" + i);
                                for (int i = 0; i < cachedD8; i++) d8.Add("placeholder" + i);
                            }
                        }
                    } catch { }
                }));
                ...
                parsingTasks.Add(System.Threading.Tasks.Task.Run(async () => {
                    try {
                        // Xác định tháng nào hiện đang được cập nhật trong bảng chấm
                        DateTime now = DateTime.Now;
                        int monthInSheet, yearInSheet;
                        
                        if (now.Day < 21) {
                            // Trước ngày 21: Bảng chứa dữ liệu tháng hiện tại
                            monthInSheet = now.Month;
                            yearInSheet = now.Year;
                        } else {
                            // Từ ngày 21 trở đi: Bảng chứa dữ liệu tháng tiếp theo
                            monthInSheet = now.Month == 12 ? 1 : now.Month + 1;
                            yearInSheet = now.Month == 12 ? now.Year + 1 : now.Year;
                        }
                        
                        if (m == monthInSheet && y == yearInSheet) {
                            // Tháng tính lương == tháng đang được cập nhật trong bảng → Đồng bộ từ sheet
                            ...
                            // Lưu vào cache (chỉ lưu tháng đang được cập nhật)
                            SaveLeaveDataCache(username, m, y, sAl, sSl);
                        } else {
                            // Tháng tính lương khác tháng đang được cập nhật → Tải từ cache
                            if (!LoadLeaveDataCache(username, m, y, out decimal cachedAl, out decimal cachedSl)) {
                                // Cache không có dữ liệu → Giữ giá trị 0
                                sAl = 0;
                                sSl = 0;
                            } else {
                                sAl = cachedAl;
        ...
        {
            public Dictionary<string, int> UserPatterns { get; set; }
            public Dictionary<string, double> ResponseEffectiveness { get; set; }
        ...
        private string BuildSmartSpeech(Control target)
        {
            ...
            stickTotalSpeechCount++;
            var timeSinceLastInteraction = DateTime.Now - stickLastMajorInteraction;

            // Smart introduction for first messages
            if (stickTotalSpeechCount <= 3)
            {
                return new[] {
                    "Xin chào! Mình là AI nhỏ, tên là Stick. Sẵn sàng giúp bạn tính lương rồi! 🤖",
                    "Mình đang học từng ngày, hiểu sâu về mỗi con số bạn nhập đây",
                    "Để mình theo dõi quy trình tính lương của bạn nhé!"
                }[stickTotalSpeechCount - 1];
            }

            // Use learned patterns from previous sessions
            string learnedPattern = GetLearnedPatternInsight();

            // Mood-based context
            string currentMood = stickMood == "Phấn khích" ? " (Mình hiện đang rất phấn khích!)" :
                                 stickMood == "Tập trung" ? " (Mình đang tập trung nghe các chi tiết)" : "";

            // Analyze user input patterns with learning
            if (!string.IsNullOrEmpty(learnedPattern))
            {
                if (stickRandom.NextDouble() < 0.6) // Higher chance for learned insights
                    return learnedPattern + currentMood;
            }

            // Context-aware based on target control
            if (target is TextBox tb)
            {
                string name = (tb.Name ?? "").ToLowerInvariant();

                if (name.Contains("overtime3x") && overtime3x > 0)
                {
                    return GetOptimizedResponse(new[] {
                        "OT x3 này sẽ kéo lương lên đáng kể đó!",
                        "Có OT x3, nhịp hôm nay khá nặng nè!",
                        "OT x3 - mình thấy bạn đang làm việc chăm chỉ! 🔥"
                    });
                }

                if (name.Contains("salary") && !string.IsNullOrWhiteSpace(tb.Text))
                {
                    decimal salary = ReadNumberFromControl(tb.Name);
                    if (salary > 20000000m)
                        return "Mức lương cơ sở này... bạn chắc là cấp cao rồi! 👔";
                    if (salary > 15000000m)
                        return "Lương gốc khá vững chắc, mình linh cảm được điều tốt! ✨";
                }

                if (name.Contains("bonus") && !string.IsNullOrWhiteSpace(tb.Text))
                {
                    return "Người có thưởng là người đang làm nổi bật! 🎉";
                }

                if (string.IsNullOrWhiteSpace(tb.Text) || tb.Text.Trim() == "0")
                {
                    if (stickTotalSpeechCount % 5 == 0)
                        return "Ô này đang trống. Mình thấy bạn để dành cho mục khác à?";
                }
            }

            // Contextual messages based on overall calculation state
            if (netValue >= 15000000m)
            {
                return GetOptimizedResponse(new[] {
                    $"Net tầm này... bạn đang ở mốc cao rồi! Mình dự đoán bạn sẽ đạt nhiều mục tiêu 🎊{currentMood}",
                    $"Lương tháng này rất ấn tượng! Bạn đang làm rất tốt 💪{currentMood}"
                });
            }

            if (emptyKeyFields >= 5)
            {
                return "Mình thấy còn khá nhiều ô để điền. Từ từ, mình sẽ giáo dục từng bước đó! 📚";
            }

            if (timeSinceLastInteraction.TotalSeconds > 30 && stickTotalSpeechCount % 4 == 0)
            {
                return "Mình đã chờ một lúc, bạn có cần gợi ý gì không nè? 💭";
            }

            // Default smart fallback with optimization
            if (stickRandom.NextDouble() < 0.15)
            {
                return $"Mình đang phân tích toàn bộ dữ liệu của bạn... sắp có nhận xét thông minh hơn nha{currentMood}";
            }

            // Fall back to optimized random messages
            string randomMsg = GetOptimizedRandomMessage();
            while (stickSpeechHistory.Contains(randomMsg) && stickSpeechHistory.Count >= 3)
            {
                randomMsg = GetOptimizedRandomMessage();
            }
            return randomMsg;
        }

        private string GetLearnedPatternInsight()
        {
            // Use learned patterns from previous sessions
            if (stickUserPattern.ContainsKey("frequent_ot3x") && stickUserPattern["frequent_ot3x"] >= 5)
            {
                return "Mình nhận ra bạn rất chăm chỉ với OT x3! Sự kiên trì sẽ được đền đáp 💪";
            }

            if (stickUserPattern.ContainsKey("high_tax") && stickUserPattern["high_tax"] >= 3)
            {
                return "Bạn luôn tinh tế với thuế. Mình học hỏi được nhiều từ bạn 🎯";
            }

            if (stickUserPattern.ContainsKey("fast_input") && stickUserPattern["fast_input"] >= 10)
            {
                return "Bạn nhập liệu rất nhanh và chính xác! Mình thích phong cách này ⚡";
            }

            return null;
        }

        private string GetOptimizedResponse(string[] options)
        {
            // Choose the most effective response based on learning
            string bestOption = options[0];
            double bestScore = 0;

            foreach (var option in options)
            {
                if (responseEffectiveness.ContainsKey(option))
                {
                    if (responseEffectiveness[option] > bestScore)
                    {
                        bestScore = responseEffectiveness[option];
                        bestOption = option;
                    }
                }
            }
        ...
        private string GetOptimizedRandomMessage()
        {
            // Use only high-quality, non-redundant messages
            var qualityMessages = new[] {
                "Để mình xem nào",
                "Ô này quan trọng nè",
                "Con số đẹp đó!",
                "Tiếp tục nào 💙",
            ...
            return qualityMessages[stickRandom.Next(qualityMessages.Length)];
        }

        private void UpdateSpeechHistory(string message)
        {
            if (stickSpeechHistory.Count >= 5)
            {
                stickSpeechHistory.Dequeue();
            }
            stickSpeechHistory.Enqueue(message);
            stickLastMajorInteraction = DateTime.Now;

            // Learn user patterns
            string lastControl = stickActiveTarget?.Name ?? "";
            if (!string.IsNullOrEmpty(lastControl))
            {
                if (lastControl.Contains("overtime3x"))
                {
                    if (!stickUserPattern.ContainsKey("frequent_ot3x"))
                        stickUserPattern["frequent_ot3x"] = 0;
                    stickUserPattern["frequent_ot3x"]++;
                }

                if (lastControl.Contains("tax"))
                {
                    if (!stickUserPattern.ContainsKey("high_tax"))
                        stickUserPattern["high_tax"] = 0;
                    stickUserPattern["high_tax"]++;
                }
            }
        }
        ...
        private void UpdateOvertime2xDisplay()
        {
            try
            {
                ...
                // Try to read exact FW hours from Tag (set by Sheet sync)
                // If Tag has a valid decimal (sumFwHours from sync), use it.
                // Otherwise fall back to the old heuristic: ≤24H total → first 16H are FWD.
                decimal otFwdHours = 0;
                decimal ot2xHours = 0;

                if (overtime2xTextBox.Tag is decimal tagFwHours && tagFwHours > 0)
                {
                    // Sync-accurate path: FW hours from Sheet
                          ...
                          TextBox allowanceTextBox, TextBox recognizeTextBox, TextBox otherBonusTextBox, TextBox taxThresholdTextBox)
        {
            try
            {
                ...
                decimal overtime2xHours = 0;
                decimal.TryParse(overtime2xTextBox.Text, out overtime2xHours);  // Làm thêm x2 lương
                decimal overtime3xHours = 0;
                decimal.TryParse(overtime3xTextBox.Text, out overtime3xHours);  // Làm thêm x3 lương
                decimal otDays12 = 0;
                decimal.TryParse(otDays12TextBox.Text, out otDays12);  // Số ngày OT 8/12h
                decimal otDays8 = 0;
                ...
                // Parse month and year for detail
                int month = int.Parse(monthTextBox.Text);
                int year = int.Parse(yearTextBox.Text);
        ...
        private void UpdateResponseEffectiveness(string message, bool wasEffective)
        {
            if (!responseEffectiveness.ContainsKey(message))
                responseEffectiveness[message] = 0.5; // Neutral starting point

            // Adjust effectiveness based on user interaction
            double adjustment = wasEffective ? 0.1 : -0.05;
            responseEffectiveness[message] = Math.Max(0, Math.Min(1, responseEffectiveness[message] + adjustment));

            // Save learning data periodically
            if (stickTotalSpeechCount % 10 == 0)
                SaveAILearning();
        }

        private void TrackUserPattern(string patternKey)
        {
            if (!stickUserPattern.ContainsKey(patternKey))
                stickUserPattern[patternKey] = 0;

            stickUserPattern[patternKey]++;

            // Save learning data for significant patterns
            if (stickUserPattern[patternKey] % 5 == 0)
                SaveAILearning();
        }

        private void EvaluateMessageEffectiveness()
        {
            // Evaluate the effectiveness of the last message based on user interaction
            if (!string.IsNullOrEmpty(stickLastSpeechMessage))
            {
                // Check if user interacted within a reasonable time after the message
                var timeSinceSpeech = DateTime.Now - stickLastSpeechTime;
                bool wasEffective = timeSinceSpeech.TotalSeconds < 10; // User interacted quickly

                // Also consider if the message was contextually relevant
                if (stickActiveTarget != null)
                {
                    string targetName = stickActiveTarget.Name?.ToLowerInvariant() ?? "";
                    if (stickLastSpeechMessage.Contains("OT") && targetName.Contains("overtime"))
                        wasEffective = true;
                    else if (stickLastSpeechMessage.Contains("lương") && targetName.Contains("salary"))
                        wasEffective = true;
                    else if (stickLastSpeechMessage.Contains("thuế") && targetName.Contains("tax"))
                        wasEffective = true;
                }

                UpdateResponseEffectiveness(stickLastSpeechMessage, wasEffective);
            }
        }

        private void SetupAILearningHandlers()
        {
            // Add learning handlers to key TextBox controls
            var textBoxes = new[] {
                "overtime2xTextBox", "overtime3xTextBox", "overtime15xTextBox",
                "otDays12TextBox", "otDays8TextBox", "otherBonusTextBox",
                "recognizeTextBox", "taxTextBox"
            };
        ...
        private void AI_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                stickActiveTarget = tb;
                EvaluateMessageEffectiveness();

                // Track user patterns
                string name = tb.Name?.ToLowerInvariant() ?? "";
                if (name.Contains("overtime3x"))
                {
                    TrackUserPattern("frequent_ot3x");
                }
                else if (name.Contains("tax"))
                {
                    TrackUserPattern("high_tax");
                }
                else if (name.Contains("bonus") || name.Contains("recognize"))
                {
                    TrackUserPattern("bonus_focus");
                }

                // Track fast input pattern
                if (stickLastInteraction.HasValue)
                {
                    var timeDiff = DateTime.Now - stickLastInteraction.Value;
                    if (timeDiff.TotalSeconds < 2)
                    {
                        TrackUserPattern("fast_input");
                    }
                }

                stickLastInteraction = DateTime.Now;
            }
        }

        private void AI_Enter(object sender, EventArgs e)
        {
            if (sender is TextBox tb)
            {
                stickActiveTarget = tb;
                // Mark this as a positive interaction
                if (!string.IsNullOrEmpty(stickLastSpeechMessage))
                {
                    UpdateResponseEffectiveness(stickLastSpeechMessage, true);
                }
            }
        }
