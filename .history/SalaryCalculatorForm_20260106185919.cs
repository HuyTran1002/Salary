    ...
    public partial class SalaryCalculatorForm : Form
    {
                                      ...
                                      TextBox attendanceTextBox, TextBox recognizeTextBox, TextBox otherBonusTextBox, TextBox taxThresholdTextBox)
        {
            try
            {
                ...
                // Round tax down (TowardsZero) for accuracy
                decimal taxDeduction = taxBase > 0 ? Math.Round(taxBase * taxRate, 0, MidpointRounding.TowardsZero) : 0;
                ...
                string detail = $"Chi Tiết:\n" +
                    ...
                    $"  • OT x2 ({overtime2xHours:F1} tiếng × {hourlyRate:F1} × 2): {overtime2xSalary:C0} VND\n" +
                    $"  • OT x3 ({overtime3xHours:F1} tiếng × {hourlyRate:F1} × 3): {overtime3xSalary:C0} VND\n" +
                    $"  • OT x1.5 ({overtime15xHours:F1} tiếng × {hourlyRate:F1} × 1.5): {overtime15xSalary:C0} VND{bonusInfo}{incentiveInfo}";
