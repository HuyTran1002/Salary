...
namespace SalaryCalculator
{
    ...
    public class CalculatorForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            // Button layout: 4 columns x 7 rows
            ...
            // Row 5: %, Ans, ^, =
            AddBtn("%", 0, 5, tag: "%", bgColor: opColor);
            AddBtn("Ans", 1, 5, tag: "Ans", bgColor: opColor);
            AddBtn("^", 2, 5, tag: "^", bgColor: opColor);
            AddBtn("=", 3, 5, tag: "=", bgColor: eqColor);

            // Row 6: ↑, ↓ (history navigation)
            AddBtn("↑", 1, 6, tag: "UP");
            AddBtn("↓", 2, 6, tag: "DOWN");
        ...
        private void CalcButton_Click(object sender, EventArgs e)
        {
            ...
            if (key == "UP")
            {
                ...
                {
                    historyIndex++;
                    currentInput = history[history.Count - 1 - historyIndex];
                    UpdateDisplay();
                }
                return;
            }
            ...
            if (key == "+" || key == "-" || key == "*" || key == "/")
            {
                ...
                try
                {
                    lastValue = EvaluateExpression(currentInput);
                }
                catch
                {
                    lastValue = 0;
