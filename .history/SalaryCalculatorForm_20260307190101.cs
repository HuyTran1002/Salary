                          ...
                          TextBox allowanceTextBox, TextBox recognizeTextBox, TextBox otherBonusTextBox, TextBox taxThresholdTextBox)
        {
            try
            {
                ...
                string incentiveInfo = $"\n  • Tổng phụ cấp: {allowanceTotal:C0} VND" +
                                        $"\n    - Tiền đi lại thực tế ({adjustedTravelPerMonth:C0} / {workingDays:F1} = {travelPerDayEffective:C0}) × {allowanceEligibleDays:F1}: {travelAllowance:C0} VND" +
                                        $"\n    - Tiền chuyên cần thực tế ({adjustedAttendancePerMonth:C0} / {workingDays:F1} = {attendancePerDayEffective:C0}) × {allowanceEligibleDays:F1}: {attendanceIncentive:C0} VND" +
