...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
            ...
            private string stickAction = "Đi bộ";
        ...
        private void PickNextStickDestination(Rectangle bounds)
        {
            ...
            {
                int pickIndex = stickRandom.Next(Math.Min(3, scored.Count));
        ...
        private PointF ComputeSmartStickDestination(Rectangle targetRect, Rectangle bounds, out bool climbMode)
        {
            ...
            bool highTarget = targetCenterY < groundY - 120f;
            if (highTarget)
            {
                climbMode = true;
                float sideX = targetCenterX >= stickFigurePosition.X
                    ? targetRect.Left - 14f
                    : targetRect.Right + 14f;
                float climbY = targetRect.Top + Math.Min(22f, targetRect.Height * 0.5f);
                return new PointF(sideX, climbY);
            }

            float x = targetCenterX + stickRandom.Next(-20, 21);
            float y = groundY;
        ...
        private IEnumerable<Control> GetInteractiveControls(Control root)
        {
            foreach (Control child in root.Controls)
            {
                ...
                if ((child is Label || child is TextBox || child is Button || child is CheckBox) &&
        ...
        private string BuildSmartSpeech(Control target)
        {
            if (target == null)
            {
                return stickMood == "Phấn khích" ? "Hôm nay vui quá!" : stickMessages[stickRandom.Next(stickMessages.Length)];
            }
            ...
            {
                return "Net là mục tiêu chính!";
            }
            if (t.Contains("Thuế", StringComparison.OrdinalIgnoreCase))
            {
                return "Thuế này phải để ý kỹ";
            }
            if (target is Button)
            {
                return stickMood == "Phấn khích" ? "Nút này mạnh nè, bấm đi!" : "Bấm nút này là chạy nè!";
            }
            ...
            return stickMessages[stickRandom.Next(stickMessages.Length)];
        }
        ...
        private void StickFigureHostPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            float headRadius = 7f;
            float bodyTopY = stickFigurePosition.Y - 22f + bodyBob;
            float bodyBottomY = stickFigurePosition.Y - 3f + bodyBob;
            float headCenterX = stickFigurePosition.X;
            float headCenterY = bodyTopY - 9f;

            PointF shoulder = new PointF(stickFigurePosition.X, bodyTopY + 6f);
            PointF hip = new PointF(stickFigurePosition.X, bodyBottomY);

            PointF leftHandTarget = new PointF(stickFigurePosition.X - 10f, bodyTopY + 8f + walkOffset);
            PointF rightHandTarget = new PointF(stickFigurePosition.X + 10f, bodyTopY + 8f - walkOffset);
            PointF leftFootTarget = new PointF(stickFigurePosition.X - 7f * direction, stickFigurePosition.Y + 11f);
            PointF rightFootTarget = new PointF(stickFigurePosition.X + 7f * direction, stickFigurePosition.Y + 11f + walkOffset * 0.3f);
            ...
            if (hasVisibleTarget)
            {
                ...
                float tx = touchPoint.X - shoulder.X;
                float ty = touchPoint.Y - shoulder.Y;
                ...
                if (dist > 0.001f)
                {
                    ...
                    float nx = tx / dist;
                    float ny = ty / dist;
                    PointF stretched = new PointF(shoulder.X + nx * maxReach, shoulder.Y + ny * maxReach);
            ...
            if (stickAction == "Chạy")
            {
                float sprint = (float)Math.Sin(stickWalkPhase * 1.7f) * 5f;
                leftHandTarget = new PointF(stickFigurePosition.X - 12f, bodyTopY + 8f - sprint * 0.45f);
                rightHandTarget = new PointF(stickFigurePosition.X + 12f, bodyTopY + 8f + sprint * 0.45f);
                leftFootTarget = new PointF(stickFigurePosition.X - 8f * direction, stickFigurePosition.Y + 10f + sprint);
                rightFootTarget = new PointF(stickFigurePosition.X + 8f * direction, stickFigurePosition.Y + 10f - sprint);
            }
            else if (stickAction == "Nhảy")
            {
                leftHandTarget = new PointF(stickFigurePosition.X - 8f, bodyTopY - 5f);
                rightHandTarget = new PointF(stickFigurePosition.X + 8f, bodyTopY - 5f);
                leftFootTarget = new PointF(stickFigurePosition.X - 4f, stickFigurePosition.Y + 7f);
                rightFootTarget = new PointF(stickFigurePosition.X + 4f, stickFigurePosition.Y + 7f);
            }
            else if (stickAction == "Leo trèo")
            {
                float climbWave = (float)Math.Sin(stickWalkPhase * 1.4f) * 5f;
                leftHandTarget = new PointF(stickFigurePosition.X - 5f, bodyTopY + 2f + climbWave);
                rightHandTarget = new PointF(stickFigurePosition.X + 5f, bodyTopY + 8f - climbWave);
                leftFootTarget = new PointF(stickFigurePosition.X - 5f, stickFigurePosition.Y + 8f - climbWave * 0.5f);
                rightFootTarget = new PointF(stickFigurePosition.X + 5f, stickFigurePosition.Y + 8f + climbWave * 0.5f);
            }
            else if (stickAction == "Tương tác")
            {
                float wave = (float)Math.Sin(stickGesturePhase * 1.8f) * 3f;
                leftHandTarget = new PointF(stickFigurePosition.X - 9f, bodyTopY + 8f + wave);
            }

            PointF leftElbow = ComputeJointPoint(shoulder, leftHandTarget, 8.5f, 8f, !facingRight);
            PointF rightElbow = ComputeJointPoint(shoulder, rightHandTarget, 8.5f, 8f, facingRight);
            PointF leftKnee = ComputeJointPoint(hip, leftFootTarget, 9f, 10f, !facingRight);
            PointF rightKnee = ComputeJointPoint(hip, rightFootTarget, 9f, 10f, facingRight);
            ...
            using (var headBrush = new SolidBrush(Color.FromArgb(225, 243, 255)))
            {
                ...
                g.DrawLine(accentPen, shoulder, leftElbow);
                g.DrawLine(accentPen, leftElbow, leftHandTarget);
                g.DrawLine(accentPen, shoulder, rightElbow);
                g.DrawLine(accentPen, rightElbow, rightHandTarget);

                g.DrawLine(bodyPen, hip, leftKnee);
                g.DrawLine(bodyPen, leftKnee, leftFootTarget);
                g.DrawLine(bodyPen, hip, rightKnee);
                g.DrawLine(bodyPen, rightKnee, rightFootTarget);

                g.FillEllipse(Brushes.White, leftElbow.X - 1.4f, leftElbow.Y - 1.4f, 2.8f, 2.8f);
                g.FillEllipse(Brushes.White, rightElbow.X - 1.4f, rightElbow.Y - 1.4f, 2.8f, 2.8f);
                g.FillEllipse(Brushes.White, leftKnee.X - 1.5f, leftKnee.Y - 1.5f, 3f, 3f);
                g.FillEllipse(Brushes.White, rightKnee.X - 1.5f, rightKnee.Y - 1.5f, 3f, 3f);
                g.DrawEllipse(bodyPen, leftElbow.X - 1.4f, leftElbow.Y - 1.4f, 2.8f, 2.8f);
                g.DrawEllipse(bodyPen, rightElbow.X - 1.4f, rightElbow.Y - 1.4f, 2.8f, 2.8f);
                g.DrawEllipse(bodyPen, leftKnee.X - 1.5f, leftKnee.Y - 1.5f, 3f, 3f);
                g.DrawEllipse(bodyPen, rightKnee.X - 1.5f, rightKnee.Y - 1.5f, 3f, 3f);

                g.FillEllipse(Brushes.White, leftHandTarget.X - 1.3f, leftHandTarget.Y - 1.3f, 2.6f, 2.6f);
                g.FillEllipse(Brushes.White, rightHandTarget.X - 1.3f, rightHandTarget.Y - 1.3f, 2.6f, 2.6f);
                g.FillEllipse(Brushes.White, leftFootTarget.X - 1.4f, leftFootTarget.Y - 1.4f, 2.8f, 2.8f);
                g.FillEllipse(Brushes.White, rightFootTarget.X - 1.4f, rightFootTarget.Y - 1.4f, 2.8f, 2.8f);
