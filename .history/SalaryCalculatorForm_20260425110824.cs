    ...
    public partial class SalaryCalculatorForm : Form
    {
        ...
        private async System.Threading.Tasks.Task SyncDataFromSheetAsync(string username)
        {
            try
            {
                ...
                parsingTasks.Add(System.Threading.Tasks.Task.Run(async () => {
                    try {
                        // Xác định xem bảng chấm hiện tại chứa dữ liệu tháng nào
                        // Nếu hôm nay < 21: bảng chứa dữ liệu từ 21 tháng trước đến 20 tháng hiện tại
                        // Nếu hôm nay >= 21: bảng chứa dữ liệu từ 21 tháng hiện tại đến 20 tháng tiếp theo
                        DateTime now = DateTime.Now;
                        int currentMonthInSheet = now.Day < 21 ? now.Month : (now.Month == 12 ? 1 : now.Month + 1);
                        int currentYearInSheet = now.Day < 21 ? now.Year : (now.Month == 12 ? now.Year + 1 : now.Year);
                        
                        // Chỉ đồng bộ nếu tháng tính lương == tháng dữ liệu trong bảng hiện tại
                        if (m == currentMonthInSheet && y == currentYearInSheet) {
                            ...
                            // Nếu tháng tính lương khác tháng dữ liệu trong bảng, tải từ cache
