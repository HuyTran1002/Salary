...
namespace SalaryCalculator
{
    public partial class SalaryCalculatorForm : Form
    {
            ...
            private const decimal BaseTaxThreshold = 15500000m; // Giảm trừ gia cảnh 2026: 15,500,000 VND
            private const decimal FixedThresholdAddon = 2452236m; // Các giảm trừ khác (điều chỉnh để khớp ví dụ thực tế)
        ...
        private decimal ComputeTaxThreshold(decimal baseThreshold, decimal hourlyRate, decimal overtime2xHours, decimal overtime3xHours, decimal overtime15xHours, decimal insuranceDeduction)
        {
            ...
            // Công thức tính mốc lương tính thuế 2026: Giảm trừ gia cảnh (15.5M) + Các giảm trừ khác (2.45M) + tiền bảo hiểm + ((tiền OT x2 của số tiếng OT x2)/2) + ((tiền OT x3 của số tiền OT x3)*2/3) + ((tiền OT x1.5 của số tiếng OT x1.5)*1/3) + ((tiền OT FWD)*1/12)
