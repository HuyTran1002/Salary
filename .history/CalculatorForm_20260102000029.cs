...
namespace SalaryCalculator
{
    ...
    public class CalculatorForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            // Button layout: 4 columns x 9 rows (D-pad cross at top + number pad + long equals)
            var table = new TableLayoutPanel();
            table.ColumnCount = 4;
            table.RowCount = 9;
            table.Width = 400;
            table.Height = 560;
            table.Location = new Point(60, 110);  // Dịch sang phải 50px (10 + 50 = 60)
            ...
            // Row 0: ↑ (col 1, 1 ô vuông)
            AddBtn("↑", 1, 0, tag: "UP", bgColor: navColor);

            // Row 1: ← (col 0) | → (col 2) - cân đối dấu thập
            AddBtn("←", 0, 1, tag: "LEFT", bgColor: navColor);
            AddBtn("→", 2, 1, tag: "RIGHT", bgColor: navColor);

            // Row 2: ↓ (col 1, 1 ô vuông)
            AddBtn("↓", 1, 2, tag: "DOWN", bgColor: navColor);

            // Row 3: AC, DEL, (, )
            AddBtn("AC", 0, 3, tag: "AC", bgColor: acColor);
            AddBtn("DEL", 1, 3, tag: "DEL", bgColor: acColor);
            AddBtn("(", 2, 3, tag: "(");
            AddBtn(")", 3, 3, tag: ")");
            ...
            AddBtn("%", 3, 7, tag: "%", bgColor: opColor);

            // Row 8: Ans, ^, = (dài 2 cột)
            AddBtn("Ans", 0, 8, tag: "Ans", bgColor: opColor);
            AddBtn("^", 1, 8, tag: "^", bgColor: opColor);
            AddBtn("=", 2, 8, colSpan: 2, tag: "=", bgColor: eqColor);
