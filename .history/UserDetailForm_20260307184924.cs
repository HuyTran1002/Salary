...
namespace SalaryCalculator
{
    public class UserDetailForm : Form
    {
        ...
        public UserDetailForm(UserInfo user)
        {
            ...
            AddInfoRow(6, "Tiền chuyên cần/tháng:", attendancePerMonth.ToString("N0") + " VND");
            AddInfoRow(7, "Tiền đi lại/tháng:", travelPerMonth.ToString("N0") + " VND");
            ...
            if (user.SalaryHistory != null && user.SalaryHistory.Count > 0)
            {
                ...
                {
                    bool hasDetail = user.SalaryResultHistory != null && user.SalaryResultHistory.ContainsKey(entry.Key);
                    salaryGrid.Rows.Add(
                        entry.Key,
            ...
            salaryGrid.CellClick += (s, e) =>
            {
                ...
                if (user.SalaryResultHistory != null && user.SalaryResultHistory.TryGetValue(periodKey, out string resultDetail))
                {
                    var detailForm = new SalaryHistoryDetailForm(user, periodKey, resultDetail);
